...
10. **Trending Games Digest** — Built a scheduled workflow that pulls trending board games from the BoardGameGeek API and posts them as a daily GitHub issue.
11. **ChatOps Game Lookup** — Created a `/game-lookup` slash command that lets team members look up board game details directly from a GitHub issue comment.
